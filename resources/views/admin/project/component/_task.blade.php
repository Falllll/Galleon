<div id="" class="" x-show="openTab === 2">
    ...
    <div class="w-full px-2">
        <div class="flex md:flex-row flex-col gap-y-3 min-w-full flex items-center justify-between bg-indigo-600 p-3">
            <a class="py-1 px-3 text-blue-600 bg-gray-100" href="{{ url('admin/project')}}/{{ $project->id }}/{{ 'task/create' }}">
                Tambah
            </a>
            {{-- <form action="{{ route('') }}">
                ...
                @foreach ($tasks as $data)
                    <tr>
                        ...
                        <td class="flex space-x-2 text-left py-3 px-4">
                            <a href="{{ url('admin/project/task') }}/{{ $data->id }}/{{ 'edit' }}">
                                <div
                            ...
                            <form action="{{url('admin/project/task')}}/{{ $data->id }}" method="post">
                                @method('DELETE')
                                @csrf
                                <button type="submit"
