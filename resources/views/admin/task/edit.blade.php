...
<form method="POST" action="{{route('admin.task.update', $task->id)}}" class="p-3" enctype="multipart/form-data">
    @method('PUT')
    @csrf
    <div class="bg-white rounded shadow border p-6">
                ...
                <div class="relative w-full border border-gray-400">
                    <select id="worker_id"
                        ...
                        @foreach($workers as $data)
                            @if ( old('worker_id', $task->worker_id) == $data->id )
            ...
            <div class="col-span-5">
                <div class="relative w-full border border-gray-400">
                    <select id="type_id"
                        ...
                        name="type_id">
                        @if ( old('type_id', $task->type_id) == $task->type_id )
                        <option value="{{$task->type_id}}" selected>--{{$task->type_id}}--</option>
        ...
        <div class="grid flex justify-between grid-cols-1 gap-4 my-3">
            ...
            <div class="col-span-5">
                <input id="file" type="file"
                    ...
                    name="file" value="{{old('file', $task->file)}}" />
                @if($errors->has('file'))
        ...
        <div class="grid flex justify-between grid-cols-1 gap-4 my-3">
            ...
            <div class='flex flex-col items-center justify-center w-full'>
                <label for="img"
                    ...
                    <input id="img" value="{{old('img', $task->img)}}" name="img" type='file'
        ...
        <a href="{{ url('admin/project') }}/{{ $task->project_id }}"
