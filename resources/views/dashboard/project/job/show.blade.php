            ...
            <li class="relative flex p-6 mb-2 border-0 rounded-t-inherit rounded-xl bg-gray-50">
                <div class="flex flex-col">
                    ...
                    <span class="mb-2 leading-tight text-sm">Task ID: <span
                            class="font-semibold text-slate-700 sm:ml-2">{{ $task->id }}</span></span>
                    <span class="mb-2 leading-tight text-sm">From Project:
                        <span class="font-semibold text-slate-700 sm:ml-2">
                            ...
                            @else
                                <a class="underline" href="{{ url('admin/project') }}/{{ $task->project->id }}">
                                    {{ $task->project->name }}
                                </a>
                            @endif
                        </span>
                    ...
                    <span class="mb-2 leading-tight text-sm">Task Name: <span
                            class="font-semibold text-slate-700 sm:ml-2">{{ $task->name }}</span></span>
                    ...
                    <span class="mb-2 leading-tight text-sm">Created At: <span
                            class="font-semibold text-slate-700 sm:ml-2">{{ date('d F Y', strtotime($task->created_at)) }}</span></span>
                    <span class="mb-2 leading-tight text-sm">Updated At: <span
                            class="font-semibold text-slate-700 sm:ml-2">{{ date('d F Y', strtotime($task->updated_at)) }}</span></span>
                            <span class="mb-2 leading-tight text-sm">Task File:
                                <span class="font-semibold text-slate-700 sm:ml-2">
                                    @if (empty($task->file))
                                        ...
                                        @else
                                        <a href="{{ asset('storage/doc/task/' . $task->file) }}"
                            ...
                            @else
                            <span class="w-4/5">
                                <a class="example-image-link" href="{{ asset('storage/img/task/' . $task->img) }}"
                                    data-lightbox="example-1"><img class="example-image"
                                        src="{{ asset('storage/img/task/' . $task->img) }}" alt="image-1" />
                                    </a>
                ...
                <div class="ml-auto text-right">
                    ...
                    <a href="{{ url('admin/project/task') }}/{{ $task->id }}/{{ 'edit' }}"
