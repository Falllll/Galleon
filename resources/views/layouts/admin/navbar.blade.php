...
<header class="w-full items-center bg-white py-2 px-6 hidden sm:flex z-50">
    ...
    <div x-data="{ isOpen: false }" class="relative w-1/2 flex justify-end">
        ...
        <div x-show="isOpen" class="absolute w-32 bg-white rounded-lg shadow-lg py-2 mt-16">
            <a href="{{ url('admin/profile') }}" class="block px-4 py-2 account-link hover:text-white">Account</a>
...
<header x-data="{ isOpen: false }" class="w-full bg-sidebar py-5 px-6 sm:hidden">
    ...
    <nav :class="isOpen ? 'flex': 'hidden'" class="flex flex-col pt-4">
        ...
        <a href="{{ url('admin/profile') }}" class="flex items-center text-white opacity-75 hover:opacity-100 py-2 pl-4 nav-item">
